...
const sendEmail = async ({ to, subject, text, html, ...rest }) => {
  try {
    ...
    if (res) {
      ...
      const mail = {
        //sender access
        from: '"VAS Store" <no-reply@accounts.vas-store.com>',
        //receiver access
        to,
        //subject
        subject,
...
const htmlSignupAccount = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Mã xác nhận đăng ký tài khoản cho website VAS Store của anh (chị).<br />
        Cảm ơn vì đã ghé thăm VAS Store <3
    </h2>
...
const htmlResetPassword = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Cửa hàng VAS Store đã nhận được yêu cầu lấy lại mật khẩu từ quý khách.<br />
